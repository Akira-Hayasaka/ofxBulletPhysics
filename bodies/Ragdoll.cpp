...
void RagDoll::render(btDynamicsWorld* m_dynamicsWorld) {
	
	for (int i = 0; i < BODYPART_COUNT; i++) {		
		
		btCollisionShape* shape = m_shapes[i];
		
		btRigidBody* body = m_bodies[i];
		btScalar m[16];
		...
		drawer.drawOpenGL(m, shape, btVector3(1, 0, 0), 0, aabbMin, aabbMax, ofxBulletStaticUtil::ofxVec4ToBtVec4(colors[i]));
...
vector<ofxVec3f> RagDoll::getPartsShapeDimention() {
	...
	for (int i = 0; i < BODYPART_COUNT; i++) {
		...
		ret.push_back(ofxBulletStaticUtil::btVec3ToOfxVec3(dimention));
	}
	return ret;
	
}

vector<ofPoint> RagDoll::getPartsRotDegree() {
	...
	for (int i = 0; i < BODYPART_COUNT; i++) {
		...
		ret.push_back( ofPoint(ofxBulletStaticUtil::radToDeg(fAngX), 
